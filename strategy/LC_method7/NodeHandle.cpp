...
void NodeHandle::ros_comms_init()
{
    ...
    PATH = node->advertise<std_msgs::Float32MultiArray>(PATH_TOPIC, 1000);
    MOVE = node->advertise<std_msgs::Bool>(MOVE_TOPIC, 1000);
...
void NodeHandle::setLocationPoint(LocationStruct *LocationPoint)
{
    ...
    for(int i=0; i<sizeof(_Location->LocationPoint) / sizeof(Point); i++){
        _Location->LocationPoint[i].x=0;       
        _Location->LocationPoint[i].y=0;
    }
}
...
void NodeHandle::pubPath(Environment *Env){
    ...
    if(_Location->LocationPoint[0].x!=0&&_Location->LocationPoint[0].y!=0){
        for(int i = 0;i < Env->Path.size;i++){
            PointMsg.data.push_back((double)(Env->Path.TargetPoint[i].x));
            PointMsg.data.push_back((double)(Env->Path.TargetPoint[i].y));
        }
        PATH.publish(PointMsg);
    }
}
void NodeHandle::pubMove(){
    std_msgs::Bool msg;
    if(_Env->GameState==9){
        msg.data=true;
        MOVE.publish(msg);
