...
void Strategy_nodeHandle::ros_comms_init(){
    ...
    //role subscriber
    robot_1_role_sub = n->subscribe<std_msgs::Int32>(robot_role_prefix  +"1"+robot_role_suffix,1,&Strategy_nodeHandle::robot_1_role_fun,this);
    ...
    Vision_Two_point = n->subscribe<vision::Two_point>(Vision_Two_point_Topic,1,&Strategy_nodeHandle::subVision_Two_point,this);
...
////Transfer vector to [0,100]
    if(Vdis==0)
        ...
        VTdis = (VTdis_max-VTdis_min)*(cos(M_PI*((Vdis-Vdis_min)/(Vdis_max-Vdis_min)-1))+1)/2+VTdis_min;
////Transfer yaw to [0,100]
    if(fabs(angle)<0.1)
        ...
        Tangle=(Tangle_max-Tangle_min)*(cos(M_PI*((fabs(angle)-angle_min)/(angle_max-angle_min)-1))+1)/2+Tangle_min;
//// [-100,100]
    if(angle<0)
        Tangle = -Tangle;
        if(IsVectorZero){
