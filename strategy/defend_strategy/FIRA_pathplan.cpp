...
//===================
    ...
    double opgoal_dis_error = 0.1;
    double opgoal_middle_offset = opgoal_right - opgoal_left;
...
//=============================
    ...
    pullback_speed = max_speed;

   // if(opgoal_dis > pullback_opgoal_dis){
    if(opgoal_dis > 1){
        x = -pullback_speed *sin(opgoal_angle*deg2rad);
        y = pullback_speed *cos(opgoal_angle*deg2rad);
        //position = pullback_M;
        //direction = pullback_M;
        rotAngle = 0;
    ...
    //std::cout<<opgoal_angle_reverse<<std::endl;
    /*
    if(fabs(opgoal_dis-0.65) > opgoal_dis_error && abs(opgoal_angle_reverse) < 10){
        if(opgoal_dis>0.65){
            env.home[r_number].v_y = -opgoal_dis;
        }
        else{
            env.home[r_number].v_y = opgoal_dis;
        }
    }*/
    if(fabs(opgoal_middle_offset) > opgoal_dis_error && abs(opgoal_angle_reverse) < 10){
        if(opgoal_middle_offset > 0){
            env.home[r_number].v_x = -0.2;
        }else{
            env.home[r_number].v_x = 0.2;
        }
    }
...
void FIRA_pathplan_class::strategy_Goalkeeper_block(int r_number){
    ...
    //ball_angle= opgoal_angle-180;
    double ball_to_opgoal_dis;//angle?
    ...
    double close_to_opgoal_dis = Goalkeeper[5];
    ...
    if(opgoal_dis < close_to_opgoal_dis){
        rotAngle = 8;
    }else{
        rotAngle = -15;
    }
    ...
    if(angle_range == 0){
        if(ball_to_opgoal_dis > 2){
            angle_range = 20;
        }else{
            angle_range = 8 * ball_to_opgoal_dis;
        }
    }
    ...
    if(opgoal_dis > pullback_opgoal_dis && ball_to_opgoal_dis > pullback_ball_to_opgoal_dis){
        ...
        }else if(ball_angle < opgoal_angle_reverse + 10 ){ // +10
            direction = R;
        }else{
            direction = stop;
        }
        ...
        position = R;
        if(ball_angle > opgoal_angle_reverse - 10){ // -10
            rotAngle = -rotAngle;
            direction = L;
    ...
    switch(direction){
    case L:
        x = -speed * sin(opgoal_edge_angle_L*deg2rad);
        y = speed * cos(opgoal_edge_angle_L*deg2rad);
        break;
    case R:
        x = - speed * sin(opgoal_edge_angle_R*deg2rad);
        y = speed * cos(opgoal_edge_angle_R*deg2rad);
        break;
    case pullback_M:
        x = -pullback_speed *sin(opgoal_angle*deg2rad);
        y = pullback_speed *cos(opgoal_angle*deg2rad);
    ...
    // std::cout << "opgoal_edge_angle_R = " << opgoal_edge_angle_R << std::endl;
...
void FIRA_pathplan_class::strategy_Goalkeeper_push(int r_number){
    ...
    if(ball_angle > opgoal_angle_reverse){
        push_L = 1;
    }else{ 
        push_L = -1;
    }
...
*/
    ...
    if(ball_angle == 999){
        yaw = 5 * (opgoal_angle_reverse>0 ? 1:-1);
    }else{
        env.home[r_number].v_x = vectornt(0);
        env.home[r_number].v_y = vectornt(1); 
        if(fabs(ball_angle) > 5){
            yaw = ball_angle*2;
        }else{
            yaw = ball_angle;
        }
    ...
    std::cout << "push -> " << rotAngle << std::endl;
...
void FIRA_pathplan_class::strategy_Goalkeeper_goalkick(int r_number){
    ...
    if(opgoal_dis < 0.8){
   	x = 1.2 * sin(ball_angle * deg2rad);
    	y = 1.2 * cos(ball_angle * deg2rad);
        direction = M;      
        ...
        x = -0.3 * sin(opgoal_edge_angle_L*deg2rad);
        y = 0.3 * cos(opgoal_edge_angle_L*deg2rad);
    	rotAngle = -18;
        direction = L;
    }else{      //R
        x = - 0.3 * sin(opgoal_edge_angle_R*deg2rad);
        y = 0.3 * cos(opgoal_edge_angle_R*deg2rad);
    	rotAngle = 18;
        direction = R;
    }
    Vector2d vectorbr(x, y);
...
void FIRA_pathplan_class::strategy_Goalkeeper_shootblock(int r_number){
    ...
    std::cout << "ball_direction = " << ball_direction << std::endl;
    std::cout << "shootblock -> " << direction << std::endl;
}
