...
void FIRA_pathplan_class::personalStrategy(int robotIndex,int action){
        switch(action){
            ...
            case action_Defend_Goal:
                ...
                break;
            case action_Straight_Leave_Ball:
                strategy_Straight_Leave_Ball(robotIndex);
                break;
        }
            ...
            if(Strategy_Selection[4]!=1){
                if(fabs(gla_angle)<=General_PathPlan[2]){
                    ...
                    if(left_right==1){
                        ...
                        env.home[r_number].v_yaw = left_right_angle;
                ...
                if(fabs(gla_angle-goal_max)>=fabs(gla_angle-goal_min)){//shoot right
                    printf("shoot right!!!!\n");
                    left_right_angle = goal_min + fabs(goal_angle-goal_min)*0.4;
                    if(left_right_angle>180){
                        left_right_angle = left_right_angle - 360;
                    }else if(left_right_angle<-180){
                        left_right_angle = left_right_angle + 360;
                    ...
                    printf("shoot left!!!!\n");
                    left_right_angle = goal_max - fabs(goal_angle-goal_max)*0.4;
                    if(left_right_angle>180){
                        left_right_angle = left_right_angle - 360;
                    }else if(left_right_angle<-180){
                        left_right_angle = left_right_angle + 360;
    ...
    switch(action){
        ...
        case 1:
            ...
            if(des_angle < 15){
                ...
                env.home[Robot_index].v_yaw = double(yaw_speed/2.5);
            }
            printf("action=%d\n",action);
            printf("right_goal_angle=%f\n",right_goal_angle);
            printf("left_goal_angle=%f\n",left_goal_angle);
...
//    double goal_min_dis = env.home[r_number].goal_edge.min_dis;
    ...
    double opgoal_max_dis = env.home[r_number].op_goal_edge.max_dis;// right face goal
    double opgoal_min_dis = env.home[r_number].op_goal_edge.min_dis;//left face goal
    double transform_angle_opdr =opgoal_max + 90;
    double inv_angle_opdr = op_angle_dr + 180;
    ...
    if(op_distance_dr>=2.5){
        ...
        if(fabs(inv_angle_opdr)<5){
            env.home[r_number].v_yaw = inv_angle_opdr;
            env.home[r_number].v_x = 0;
            env.home[r_number].v_y = 0;
            if(opgoal_max<0&&opgoal_min>=0){
                printf("good position\n");
            }
            if(opgoal_max_dis>opgoal_min_dis){
